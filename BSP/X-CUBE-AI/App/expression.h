...
#ifndef __AI_EXPRESSION_H__
#define __AI_EXPRESSION_H__
#pragma once

#include "ai_platform.h"
#include "ai_platform_interface.h"

#define AI_EXPRESSION_MODEL_NAME          "expression"

#define AI_EXPRESSION_ACTIVATIONS_ALIGNMENT   (4)

#define AI_EXPRESSION_IN_NUM       (1)
#define AI_EXPRESSION_IN { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_U8, 48, 48, 1, 1, NULL), \
}
#define AI_EXPRESSION_IN_SIZE { \
  (48 * 48 * 1), \
}
#define AI_EXPRESSION_IN_1_SIZE  (48 * 48 * 1)
#define AI_EXPRESSION_IN_1_SIZE_BYTES  ((48 * 48 * 1) * 1)





#define AI_EXPRESSION_OUT_NUM      (1)
#define AI_EXPRESSION_OUT { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_U8, 1, 1, 7, 1, NULL), \
}
#define AI_EXPRESSION_OUT_SIZE { \
  (1 * 1 * 7), \
}
#define AI_EXPRESSION_OUT_1_SIZE  (1 * 1 * 7)
#define AI_EXPRESSION_OUT_1_SIZE_BYTES  ((1 * 1 * 7) * 1)

#define AI_EXPRESSION_N_NODES (12)
 ...
 */
AI_API_ENTRY
ai_bool ai_expression_get_info(
 ...
 */
AI_API_ENTRY
ai_error ai_expression_get_error(ai_handle network);
 ...
 * using @ref ai_expression_get_error
 */
AI_API_ENTRY
ai_bool ai_expression_init(
 ...
 * header generated by the code generation tool. See AI_EXPRESSION_*
 * defines into file @ref expression.h for all network sizes defines
 *
 * @param network an opaque handle to the network context
 ...
 */
AI_API_ENTRY
ai_i32 ai_expression_forward(
  ai_handle network, const ai_buffer* input);

AI_API_DECLARE_END

#endif /*__AI_EXPRESSION_H__*/
